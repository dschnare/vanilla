...
exports.data = function (phase) {
  return {
    ...
    process: function (file, options, helpers, callback) {
      ...
      data.url = PATH.relative(PATH.resolve(options.webRoot || '.'), data.dest).replace(/\\/g, '/');
      data.files = files;
...
// Interpolate files using Lo-Dash templates.
exports.interpolation = function (phase) {
  return {
    ...
    process: function (file, options, helpers, callback) {
      ...
      data.partial = function (name) {
        if (partials[name]) {
          return _.template(partials[name], data, options.lodashTemplateSettings || null);
        }
        return '';
      };
      data.self = function () {
      ...
      file.content = _.template(file.content, data, options.lodashTemplateSettings || null);
      callback(null, file);
    }
  };
};