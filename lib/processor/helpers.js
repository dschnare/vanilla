...
exports.generateHelpers = function (context, vanilla) {
  return {
    ...
    processFile: function (filePath, options, callback) {
      var files, opt;
      ...
      opt = context.extensions[PATH.extname(filePath).substring(1)] || context.extensions.defaults;
      opt = context.helpers.extend({}, opt, options);
      
      vanilla.processFiles(files, context, opt, function (error, files) {
        if (error) {
          callback(error);
        } else {
