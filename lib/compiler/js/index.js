...
// getImports(filename, imports, visited, srcToken, callback)
function getImports(filename, imports, visited, srcToken, callback) {
  'use strict';
  var alreadyVisited;
  ...
  fs.readFile(filename, 'utf8', function (error, scriptText) {
    ...
    function next() {
      ...
      if (token.name === 'import') {
        ...
        getImports(importedFilename, imports, visited, token, function (error, imports) {
          if (error) {
            callback(error);
          } else {
            next();
