var wrench, fs, path, createStringReader, createTokenizer; 

wrench = require('wrench');
fs = require('fs');
path = require('path');
createStringReader = require('../lib/stringreader').createStringReader;
      ...
      if (mode === 'compress') {
        ...
        minify(output, function (error, output) {
          if (error) {
            ...
            wrench.mkdirSyncRecursive(options.js.output, 0777);
  ...
  switch (mode) {
    case 'debug':
      return imports.map(function (incl) {
        return '<script src="/' + path.relative(root, incl.filename) + '" type="text/javascript"></script>';
      }).join('\n') + '\n';
      break;
    default:
      return imports.map(function (incl) {
...
function processDirectives(script) {
  ...
  script = script.replace(/\/\/#DEBUG\s+(.|\s)+?\/\/#END/ig, '');
