        ...
        if (error) {
          ...
          parseExtends(text, mode, options, file, function (error, result) {
            ...
            if (error) {
              ...
              result = Hogan.render(result, options.html.context, options.html.partials || {});
              fs.writeFile(outputfilename, result.trim(), function (error) {
                if (error) {
                  callback(error);
                } else {
...
function getIncludes(filename, text) {
  'use strict';
  var includes, types, typeExt, stringReader, tokenizer, token, includedFilename, ext, endToken;
  ...
  token = tokenizer.next();
  while (token.name) {
    if (token.selfClosing && types.indexOf(token.name) >= 0) {
    ...
    } else if (!token.selfClosing && token.name === 'include') {
      (function () {
        var t, data;
        ...
        ext = path.extname(includedFilename);
        if (ext !== '.html') {
          includedFilename = path.join(path.dirname(includedFilename), path.basename(includedFilename, ext) + '.html');
        }
        ...
        while (t.name) {
          if (t.name === 'include') {
            endToken = t;
            if (t.closing) {
              data = JSON.parse(text.substring(token.end, t.start).trim());
            } else {
              throw new Error('Unbalanced <v:include></v:include> elements encountered.');
            }
          }
          t = tokenizer.next();
        }
...
function expandIncludes(text, includes, mode, options, callback) {
  ...
  function next() {
    ...
    if (includes.length) {
      ...
      options.html.context = incl.data;
      vanillaCompiler.compile(incl.filename, mode, options, function (error, result, markup) {
        if (error) {
          ...
          if (incl.endToken) {
            text = text.substring(0, incl.token.start) + markup + text.substring(incl.endToken.end);
          } else {
...
function enumerateBlocks(text) {
  var stringReader, tokenizer, token, blocks, block, blockType, blockTypes;
  
  blocks = {};
  stringReader = createStringReader(text);
  tokenizer = createTokenizer(stringReader); 
  blockTypes = ['block', 'append', 'prepend', 'replace'];
 
  token = tokenizer.next();
  while (token.name) {
    if (blockTypes.indexOf(blockType || token.name) >= 0) {
      if (!blockType) blockType = token.name;
      // </v:block> or <v:block /> encountered.
      if (token.closing) {
        // Self closing element. <v:block name="" />
        if (token.selfClosing) {
          if (!token.attributes.name) throw new Error('Malformed <v:?> token: Expected name attribute.'.replace(/\?/g, blockType));
          
          block = {
            name: token.attributes.name,
            operation: blockType === 'block' ? token.attributes.operation : blockType,
            startToken: token
          };
        }
        ...
        if (block) {
          ...
          throw new Error('Unbalanced <v:?></v:?>.'.replace(/\?/g, blockType));
        }
      ...
      // <v:block> encountered.
        ...
        if (block) {
          throw new Error('Unbalanced <v:?></v:?> tokens.'.replace(/\?/g, blockType));
          ...
          throw new Error('Malformed <v:?> token: Expected name attribute.'.replace(/\?/g, blockType));
        }
      }
    }
