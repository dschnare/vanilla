    ...
    if (token.name === 'block') {
      ...
      if (token.closing) {
        ...
        if (block) {
          ...
          block.body = text.substring(block.beginToken.end, block.endToken.start);
...
function replaceBlocks(blocks, text) {
  ...
  Object.keys(blocks).reverse().forEach(function (name) {
    ...
    text = text.substring(0, block.beginToken.start) + block.body + text.substring(block.endToken.end);
  });
  return text;
}