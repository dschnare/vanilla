...
exports.compile = function (file, mode, options, child, callback) {
  ...
  parseIncludes(file, mode, options, function (error, text) {
    if (error) {
      ...
      parseExtends(text, mode, options, child, file, function (error, result) {
        if (error) {
          callback(error);
        } else if (options.html.including) {
          callback(null, result);
        } else {
          outputbasename = options.html.basenameOverride || path.basename(file, '.html');
...
function parseExtends(text, mode, options, child, filename, callback) {
  var extendsTokens, extendsToken, blocks;
  
  extendsTokens = getExtendsTokens(text);
  
  if (extendsTokens.length === 1) {
    extendsToken = extendsTokens[0];
    text = replaceLine(text, getLineWithIndex(extendsToken.begin));
    blocks = enumerateBlocks(text);
    applyChildBlocks(blocks, child);
    
    options = extendOptions(options);
    options.html.including = false;
    options.html.basenameOverride = path.basename(filename, '.html');
    
    exports.compile(layoutFilename, mode, options, blocks, callback);
...
function enumerateBlocks(text) {
  ...
  while (token.name) {
    if (token.name === 'block') {
      ...
      if (token.closing) {
        if (block) {
          block.endLine = getLineWithIndex(token.begin);
          block.bodyEnd = token.begin;
      ...
      // <v:block> encountered.
        ...
        if (block) {
          ...
          block = {
            ...
            beginLine: getLineWithIndex(token.begin),
            bodyBegin: token.end
          };
        } else {
...
function replaceBlocks(blocks, text) {
  ...
  Object.keys(blocks).reverse().forEach(function (name) {
    ...
    text = text.substring(0, block.beginLine.begin) + block.body + text.substring(block.endLine.begin);
  });
  return text;
}