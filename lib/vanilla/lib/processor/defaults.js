var _, LIB, YUI, PATH, FS, MKDIRP, TMP;
      ...
      if (options.mode === 'compress') {
        YUI.compress(file.content, {
          ...
          if (error) {
            ...
            file.content = result;
            file.dest = file.dest ? PATH.join(PATH.dirname(file.dest), PATH.basename(file.dest, extname) + '.min.' + extname) : '';
            callback(null, file);
          }
        });
      } else {
